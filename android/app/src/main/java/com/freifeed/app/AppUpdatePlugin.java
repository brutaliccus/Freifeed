...
public class AppUpdatePlugin extends Plugin {
    ...
    private static final String PREFS = "freifeed_app_update_events";
    ...
    public void showUpdateAvailable(PluginCall call) {
        ...
        AppUpdateNotifier.show(getContext(), title, body, releaseKey, downloadUrl, authToken);
        call.resolve();
    }
    ...
    static void startDownloadFromPending(Context context) {
        String url = AppUpdatePendingStore.getDownloadUrl(context);
        String authToken = AppUpdatePendingStore.getAuthToken(context);
        if (url.isEmpty() || authToken.isEmpty()) {
            dispatchUpdateNow(context);
            return;
        }
        new Thread(
            () -> {
                try {
                    File apk = downloadApkFile(context, url, authToken, null);
                    var bridge = instance != null ? instance.getBridge() : null;
                    var activity = bridge != null ? bridge.getActivity() : null;
                    if (activity != null) {
                            ...
                            () -> {
                                try {
                                    ...
                                    dispatchUpdateFailed(
                                        context,
                                        e.getMessage() != null ? e.getMessage() : "Install failed"
                                    );
                                }
                            }
                        );
    ...
    private static void dispatchUpdateNow(Context context) {
        JSObject data = new JSObject();
        data.put("releaseKey", AppUpdatePendingStore.getReleaseKey(context));
        AppUpdatePendingStore.clear(context);
        if (instance != null) {
            instance.notifyListeners("appUpdateNow", data, true);
            return;
        }
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .edit()
            .putString(PENDING_UPDATE, data.toString())
            .apply();
    }

    private static void dispatchUpdateFailed(Context context, String message) {
        JSObject data = new JSObject();
        data.put("message", message);
        if (instance != null) {
            instance.notifyListeners("appUpdateFailed", data, true);
            return;
        }
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .edit()
    ...
    private static File downloadApkFile(
        ...
        File out = new File(dir, "freifeed-update.apk");
    ...
    private static void launchPackageInstaller(Context context, File apk) throws Exception {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            if (!activity.getPackageManager().canRequestPackageInstalls()) {
                ...
                throw new Exception(
                    "Allow \"Install unknown apps\" for Buba in Settings, then tap Download again."
                );
            }
        }
